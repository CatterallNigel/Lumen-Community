...
### 3.2 Servire Standardisation

Servire should be updated at the same time so that its component presentation and configuration reflect the new family structure.

Servire remains the operational control plane. It should expose the individual Lumen components rather than treating the existing orchestrator as synonymous with the entire platform.
...
4. Standardise Servire and configuration around the Lumen family/component model.
5. Verify the complete existing stack after these changes.

### Phase 2 — Rogare

6. Develop Lumen Rogare as a separate conversational client.
7. Route Rogare through Pontis.
8. Establish ACP tool discovery/routing through Pi.
9. Validate Rogare as a tool-less client using externally supplied tools.

### Phase 3 — Fiducia

10. Remove repeated-run responsibility from Repetere specifications.
11. Develop Fiducia replay scheduling/orchestration.
12. Support replay execution counts and periodic execution.
13. Accumulate execution evidence and history.
14. Establish the initial evidence model for calibrated trust.

### Phase 4 — Assess

15. Review and formalise checkpoint semantics.
16. Resolve outstanding protocol/content classification issues such as `\obt`.
17. Develop Assess behavioural evaluation.
18. Evaluate answer sufficiency and behavioural outcomes.
19. Feed Assess evidence into Fiducia.

### Phase 5 — Integrated Reasoning Assurance

20. Build the combined Replay → Assess → Fiducia evidence cycle.
21. Surface evidence, confidence/trust and behavioural history through Servire.
22. Investigate semantic replay equivalence rather than strict textual equivalence where appropriate.
23. Develop longer-lived continuity/evidence graphs.

### Phase 6 — Public Lumen Site and Demonstration

24. Build `lumen.illuminates.one` after Assess reaches a useful level of maturity.
25. Distil the existing engineering and architectural documentation into the public story.
26. Provide an interactive demonstration, potentially using replay-driven scenarios rather than unrestricted public prompting.
27. Use a separate demonstration data store so historical runs can be managed independently.
28. Expose selected Servire capabilities while hiding operationally sensitive controls.
